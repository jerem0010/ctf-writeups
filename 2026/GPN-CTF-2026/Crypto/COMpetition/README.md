...
def winning_move(server_choice: str) -> str:
    ...
    if server_choice == "scissors":
        return "rock"
    raise ValueError(f"unknown server choice: {server_choice}")
...
with socket.create_connection((HOST, PORT)) as raw_sock:
    with context.wrap_socket(raw_sock, server_hostname=HOST) as s:
        for i in range(100):
            ...
            commitment = sha256(blob).hexdigest()

            s.sendall(commitment.encode() + b"\n")
            ...
            if "I choose rock." in text:
                choice = winning_move("rock")
            elif "I choose paper." in text:
                choice = winning_move("paper")
            elif "I choose scissors." in text:
                choice = winning_move("scissors")
            else:
                raise Exception("Could not parse server choice")
